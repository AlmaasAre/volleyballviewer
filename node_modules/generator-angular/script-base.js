...
Generator.prototype.addScssToStyle = function (script) {
  try {
    var appPath = this.env.options.appPath;
    var fullPath = path.join(appPath, '/styles/_components.scss');
    angularUtils.rewriteFile({
      file: fullPath,
      needle: '// <--component styles automatically inserted here-->',
      splicable: [
        '@import \'' + scss + '\';'
      ]
    });
  } catch (e) {
    console.log('\nUnable to find '.yellow + fullPath + '. Reference to '.yellow + script + '.scss ' + 'not added.\n'.yellow);
  }
};
